...
/// Interface for stored tags and tag color mapping. Tags are keyed by ID (index); colors are keyed by tag name.
pub trait StoredTagStore: Send + Sync {
    ...
    /// Returns the tag name -> color index mapping (from tag_color_mapping table).
    fn get_tag_color_mapping(&self) -> Result<TagColorMapping, Box<dyn std::error::Error + Send + Sync>>;

    /// Adds one stored tag and its color. Persists to stored_tags and tag_color_mapping (by name).
    fn add_stored_tag(
        &mut self,
        tag: StoredTag,
        color_index: u8,
    ...
    /// Removes a tag by name from stored_tags and from tag_color_mapping.
