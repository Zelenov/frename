...
#
#   packaging/linux/build-appimage.sh <version> <path to frename binary> <output AppImage path>
#
# Needs: the GStreamer plugin packages the app should carry (as in .github/workflows), patchelf,
# file, imagemagick. linuxdeploy, its GStreamer plugin and the AppImage runtime are downloaded at
# pinned versions and checked against their SHA-256.
set -euo pipefail
...
# Otherwise linuxdeploy's AppImage plugin downloads the moving "continuous" runtime.
fetch "$runtime_url" runtime-x86_64 "$runtime_sha256"
export LDAI_RUNTIME_FILE="$work/runtime-x86_64"
