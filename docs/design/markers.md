...
Implemented in one PR (markers and the batch action together). The owner asked for the design
"as the agent sees it"; the choices that differ from the draft reviewed in PR #20 (5307bd1):

- **Known set:** one set of GUIDs for the session in `FolderWorkspace`, not one per `FileId`:
  GUIDs are UUIDs, unique across files, so one set answers "did frename read or write this marker"
  for every file and survives the `FileId`s a folder scan renews. It reaches the core as the
  `known` argument of `FileTagger::save_markers`; batch actions pass an empty set.
- **Markers are written by their own call.** `FileTaggerBackend::save` does not touch markers.
  `apply_file_updated` calls `FileTagger::save_markers(path, markers, known)` before
  `save_and_reparse` (so before the rename), and gets a `Result` back. Batch actions, tag moves
  and every other save therefore can never write or delete markers.
- **Failed writes** keep the markers in memory by `FileId` (not by path) until the next save of
  that file; the file list shows a red ✕ with a tooltip, and the controls bar says
  `Markers not saved: the file is read-only or in use`. A folder scan drops the memory. There is
  no close prompt.
- **Not in this PR:** saving the open file on window close or folder change (a bug independent of
  markers, for its own issue); the separate read-only check when a file opens (a read-only file
  fails its write like a file in use).
- **`TagList::reinitialize_from_snapshot`** keeps the current markers, so paste, its undo and an
  in-place rename never bring back older markers; `submit_rename` and `paste_tags` need no
  marker code.
- **Same file = same `FileId`**, in `FileWorkspace::set_file` and `apply_file_opened`, as the
  draft asked. Another file reads its markers from disk in `set_file`.
- **Batch results** use the existing Done / Skipped / Failed statuses; details go to the log.
- **Demo mode:** `snaps`/`snap_image` became `markers = ["0:06.120 — City lights"]` (the line
  format below) and `marker_list = true`.

Open questions 1–6 were taken as recommended.
